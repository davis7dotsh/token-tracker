defmodule TokenTracker.Network do
  ...
  def start(config, opts) when config.role in ["host", "client"] do
    if Node.alive?() do
      ...
      Application.put_env(:kernel, :inet_dist_listen_min, config.distribution_port)
      Application.put_env(:kernel, :inet_dist_listen_max, config.distribution_port)

      options = %{
        name_domain: Config.name_domain(config),
        hidden: config.role == "client",
        dist_listen: config.role == "host"
      }
