defmodule TokenTracker.History.Scanner do
  @moduledoc false

  alias TokenTracker.{Counters, Hash, Project, Storage}

  def collect(agent, label, roots, parser, parser_version) do
    files =
      roots
    ...
    Enum.reduce(files, initial_result(agent, label, length(files)), fn path, result ->
      scan_file(path, agent, parser, parser_version, result)
    end)
  end

  defp initial_result(agent, label, total_files) do
    %{
      agent: agent,
      label: label,
  ...
  defp scan_file(path, agent, parser, parser_version, result) do
    with {:ok, before_stat} <- File.stat(path, time: :posix) do
      ...
      if Storage.checkpoint_current?(path_hash, before_stat, parser_version) do
        ...
        persist_scan(path, path_hash, before_stat, agent, parser, parser_version, result)
      end
    else
  ...
  defp persist_scan(path, path_hash, before_stat, agent, parser, parser_version, result) do
    parsed = parse_file(path, path_hash, before_stat, agent, parser)
    checkpoint = stable_checkpoint(path, path_hash, before_stat, parser_version)
  ...
  defp parse_file(path, path_hash, stat, agent, parser) do
    ...
    parsed =
      ...
      |> Enum.reduce(parse_state(path_hash), fn line, state ->
        parse_line(line, state, fallback_time, agent, parser)
      end)
  ...
  defp parse_line(line, state, fallback_time, agent, parser) do
      ...
      case Jason.decode(line) do
        {:ok, record} ->
          ...
          case parser.usage(record, context) do
            nil -> state
            usage -> put_usage(state, usage, fallback_time, agent)
          end
  ...
  defp put_usage(state, usage, fallback_time, agent) do
    session_identity = usage.session || state.file_identity
    session_key = Hash.stable([agent, session_identity])
    ...
    event_key = Hash.stable([agent, session_identity, message_identity])
    ...
    event =
      ...
      |> Map.put(:agent, agent)
      |> Map.put(:provider, usage.provider)
  ...
  defp stable_checkpoint(path, path_hash, before_stat, parser_version) do
